...
src_compile() {
	cd "${YARN_WORKDIR}"
	OUTPUT_DIR="${WORKDIR}/${P}/service/server/router/web" yarn build || die

	for file in $(find "${WORKDIR}/${P}/service/server/router/web" |grep -v png |grep -v index.html|grep -v .gz)
	do
		if [ ! -d $file ]; then
			gzip -9 $file
		fi
...
src_install() {
	default
	dobin "${WORKDIR}/${P}"/service/v2raya

	if use systemd; then
		systemd_newunit "${WORKDIR}/${P}"/install/universal/v2raya.service v2raya.service
		systemd_newunit "${WORKDIR}/${P}"/install/universal/v2raya@.service v2raya@.service
	fi

	newicon -s 512 "${WORKDIR}/${P}"/gui/public/img/icons/android-chrome-512x512.png v2raya.png
	domenu "${WORKDIR}/${P}"/install/universal/v2raya.desktop
}
